...
namespace SistemaProveedores.API.Endpoints
{
    public static class RobotEndpoints
    {
        ...
        /// Permite subir archivos, consultar pendientes y realizar inserciones masivas desacopladas usando Repositorios.
        /// </summary>
        ...
        {            
            ...
            robotGroup.MapPost("/insertar-masivo", async (List<Proveedor> loteProveedores, IProveedorRepository repo) =>
            {
                if (loteProveedores == null || !loteProveedores.Any())
                {
                    return Results.BadRequest(new { mensaje = "El lote está vacío." });
                }

                var proveedoresAInsertar = new List<Proveedor>();

                foreach (var proveedor in loteProveedores)
                {                    
                    var existe = await repo.ExisteNitAsync(proveedor.NitEmpresa);
                    if (!existe)
                    {
                        proveedor.TipoCarga = "AUTOMATICO";
                        proveedor.EstadoRegistro = proveedor.EstadoRegistro.ToUpper().Trim();
                        proveedor.NormalizarCampos();

                        proveedoresAInsertar.Add(proveedor);
                    }
                }

                if (proveedoresAInsertar.Any())
                {
                    await repo.InsertarMasivoAsync(proveedoresAInsertar);
                }
                ...
                return Results.Ok(new
                {
                    ...
                    registrosInsertados = proveedoresAInsertar.Count
                });
            });
        }
    }
